...
# 4. 使用tmux使py在后台运行 ， 在bash输入如下命令
tmux

# 5. 进入tmux，在tmux中启动程序 ，模型需要加载大约30s完成启动，在控制台可以看到启动日志
python thrift_server.py

# 6. 模型启动后，可以退出镜像 `exit`  或者直接关闭ssh终端， 模型都会正常运行
# 7. 重新ssh上去之后，执行如下命令,可以看到python程序还在正常执行
tmux attach
```
